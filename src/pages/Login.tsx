    ...
    <div className="w-full h-screen px-5 pt-16 flex flex-row justify-start items-start">
      <div className="fade-in w-1/2 h-full flex justify-center items-center">
        <form className="w-full max-w-64 flex flex-col justify-start items-start gap-2" onSubmit={handleLogin}>
          ...
          {error && <p className="mb-5" style={{ color: "red" }}>{error}</p>}

          <label htmlFor="email">Email <span className="opacity-60">(must be rit.edu)</span></label>
          <input className="w-full mb-2" type="email" name="email" id="email" onChange={(e) => setEmail(e.target.value)} pattern="^([a-zA-Z0-9_\-\.]+)@rit.edu$" required />

          <label htmlFor="password">Password</label>
          <input className="w-full mb-5" type="password" name="password" id="password" onChange={(e) => setPassword(e.target.value)} required />

          <button className="button mb-5 w-full">Log in</button>

          <p className="opacity-75">Don't have an account? <Link className="underline" to="/register">Register</Link></p>
